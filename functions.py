...
get_bosses = function(rows, boss_names){
  var boss_kills = {};
  for(i in boss_names){
...
updatedict=function(boss_kills, players, players_index, rsn, boss, kills){
...
update_rows= async function(players_index, rsn, boss, kills, rows){
    rows[players_index[rsn]][boss.toLowerCase()] = kills;
    await rows[players_index[rsn]].save();

}

create_player=async function(boss_names,players,players_index,rsn,rows){
...
get_rows=async function(sheet,rows){
